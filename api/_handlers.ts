...
export function patientResult(id: string): Result {
  ...
  if (!patient) return err(404, "patient_not_found", `No patient with id "${id}".`, "Valid ids: jane-doe, marcus-lee.");
  return { status: 200, body: patient };
}

export function payerRulesResult(payer: string): Result {
  const rules = getPayerRules(payer);
  if (!rules) return err(404, "payer_not_found", `No payer with id "${payer}".`, "Valid payers: uhc, aetna, cigna.");
...
export function validateResult(payer: string, formFields: Record<string, unknown>): Result {
  ...
  if (!rules) return err(400, "invalid_payer", `Unknown payer "${payer}".`, "Send { payer, formFields } with payer one of: uhc, aetna, cigna.");
